const CREOVA_VERSION='7.0-final-clean-start';

const $=(q,r=document)=>r.querySelector(q),$$=(q,r=document)=>[...r.querySelectorAll(q)];
const D=window.CREOVA_DATA, img=D.img;
const defaults={view:"ziel",start:"",destination:"",radius:20000000,tripType:"Urlaub",weatherPref:"Sonnig",budget:0,persons:1,dateStart:"",dateEnd:"",interests:[],selected:[],selectedPoi:[],packDone:{},customPack:[],houseDone:{},coords:{lat:51.647,lon:7.096},pois:[],weather:null,alarm:{arrival:"10:00",drive:"5 h 30 min",buffer:"30 min",wakeLead:"60 min",departure:"04:00",wake:"03:00"}};
...
").map((x,i)=>{let [name,cat,tags,key]=x.split("|");return{id:"p"+i,name,cat,tags:tags.split(","),img:(D.itemImages&&D.itemImages[name])?D.itemImages[name]:(img[key]||img.bag),qty:"1 / 1"}});
...
function load(){
  try{
    sessionStorage.removeItem("creova_boot_marker_old");
  }catch(e){}
  return {...defaults};
}
function save(){ try{sessionStorage.creova_ultimate_v7=JSON.stringify(state)}catch(e){} }
...
try{
  let q=(state.destination||state.start||"Marl").trim();
  toast("Live-Daten werden geladen...");
  let g=await geocode(q);
  state.coords={lat:g.lat,lon:g.lon};
  state.locationName=g.name;
  $("#loc").textContent=`✅ Exakter Suchort: ${g.name}`;
  await Promise.all([loadWeatherData(),loadOverpass()]);
  save();
  render();
  toast("Live-Daten geladen");
}catch(e){
  console.error(e);
  toast("Live-Daten nicht verfügbar");
}
}
...
function renderPack(){
let items=relevantPack();
let cats=[...new Set(items.map(i=>i.cat))];
let filter=$("#packFilter")?.value||"Alle Kategorien";
let search=($("#packSearch")?.value||"").trim().toLowerCase();

if($("#packFilter")){
  $("#packFilter").innerHTML='<option>Alle Kategorien</option>'+cats.map(c=>`<option>${c}</option>`).join("");
  if(cats.includes(filter)||filter==="Alle Kategorien") $("#packFilter").value=filter;
  else $("#packFilter").value="Alle Kategorien";
}

if(filter!=="Alle Kategorien") items=items.filter(i=>i.cat===filter);
if(search) items=items.filter(i=>(i.name+" "+i.cat+" "+(i.tags||[]).join(" ")).toLowerCase().includes(search));

let all=relevantPack();
let done=all.filter(i=>state.packDone[i.id]).length;

if($("#packText"))$("#packText").textContent=`${done} / ${all.length}`;
if($("#packFill"))$("#packFill").style.width=(all.length?done/all.length*100:0)+"%";

$("#packCats").innerHTML=cats.map(c=>{
  let l=all.filter(i=>i.cat===c),d=l.filter(i=>state.packDone[i.id]).length;
  return `<div class="packCat" data-catpick="${c}"><h3>${c}</h3><div class="bar"><div class="fill" style="width:${l.length?d/l.length*100:0}%"></div></div><p class="muted">${d} / ${l.length}</p></div>`
}).join("");

$("#products").innerHTML=items.map(p=>`<article class="product ${state.packDone[p.id]?"done":""}" data-pack="${p.id}">
  <div class="photo" style="background-image:url('${p.img}')"></div>
  <div class="body"><h3>${p.name}</h3><p>${p.cat} · Menge ${p.qty||"1 / 1"}</p></div>
  <div class="check">✓</div>
</article>`).join("") || '<p class="muted">Keine Artikel gefunden.</p>';

$$("[data-pack]").forEach(p=>p.onclick=()=>{
  state.packDone[p.dataset.pack]=!state.packDone[p.dataset.pack];
  save();
  renderPack();
});

$$("[data-catpick]").forEach(c=>c.onclick=()=>{
  if($("#packFilter")){
    $("#packFilter").value=c.dataset.catpick;
    renderPack();
  }
});
}
